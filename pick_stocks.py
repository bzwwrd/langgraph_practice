...
# 最新 CSV 文件索引（避免对每只股票都 glob 扫描一次目录）
_LATEST_CSV_INDEX_LOCK = threading.Lock()
_LATEST_CSV_INDEX: Optional[Dict[str, str]] = None
...
TEST_ONLY_TS_CODES = {"605599.SH"}  # 测试模式：仅处理这些股票代码；置空可恢复全量
...
AI_MAX_NEWS_ITEMS = 5
AI_QWEN_MODEL = os.environ.get("QWEN_MODEL", "deepseek-v3.2-exp")
AI_QWEN_MODELS = [
    model.strip()
...
def sort_results_by_ai_score(selected_stocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    按 AI 分数升序排序，最高分排最后。
    - 可解析为数值的 ai_score 按数值升序
    - ai_score 缺失/无效的结果排在最前面，避免打断高分靠后的阅读顺序
    """
    def _score_key(stock: Dict[str, Any]) -> Tuple[int, float, str]:
        score = stock.get("ai_score")
        try:
            if score is None or score == "":
                raise ValueError("empty ai_score")
            score_val = float(score)
            return (1, score_val, str(stock.get("ts_code", "")))
        except Exception:
            return (0, float("-inf"), str(stock.get("ts_code", "")))

    return sorted(selected_stocks, key=_score_key)
...
class KDJStrategy(StockSelectionStrategy):
    ...
    def apply(self, df):
        ...
        for idx, (date, row) in enumerate(recent_data.iterrows()):
            ...
            # kdj金叉 j值不高
            condition1 = (k_val >= d_val) and (0 <= j_val <= 40)
            # j到大负值
            condition2 = j_val <= 0
...
def get_stock_list():
    ...
    # 股票列表缓存：避免每次都请求 pro.stock_basic
    cache_enabled = os.environ.get("STOCK_BASIC_CACHE", "1").strip() not in {"0", "false", "False"}
    cache_dir = os.environ.get("STOCK_BASIC_CACHE_DIR", os.path.join(LOG_DIR, "cache"))
    os.makedirs(cache_dir, exist_ok=True)
    today = datetime.now().strftime("%Y%m%d")
    cache_path = os.path.join(cache_dir, f"stock_basic_{today}.csv")

    stock_basic: Optional[pd.DataFrame] = None
    freshness = DataFreshness(fetch_time=datetime.now(), data_may_delay=True, delay_reason="cache")

    if cache_enabled and os.path.exists(cache_path):
        try:
            stock_basic = pd.read_csv(cache_path)
            logger.info("stock_basic loaded_from_cache path=%s rows=%s", cache_path, len(stock_basic))
        except Exception as e:
            logger.warning("stock_basic cache_read_failed path=%s err=%s", cache_path, str(e))
            stock_basic = None
    ...
    if stock_basic is None:
        ...
        if cache_enabled and stock_basic is not None and not stock_basic.empty:
            try:
                stock_basic.to_csv(cache_path, index=False)
                logger.info("stock_basic cache_written path=%s rows=%s", cache_path, len(stock_basic))
            except Exception as e:
                logger.warning("stock_basic cache_write_failed path=%s err=%s", cache_path, str(e))
...
def save_stock_data_to_csv(df, ts_code, name):
    ...
    # 更新“最新 CSV 索引”缓存，避免后续选股仍读到旧文件（尤其是先更新后选股的链路）
    global _LATEST_CSV_INDEX
    with _LATEST_CSV_INDEX_LOCK:
        if _LATEST_CSV_INDEX is not None:
            prev_path = _LATEST_CSV_INDEX.get(ts_code)
            # 默认认为本次写入的是最新数据；如需更严格，可比较文件名中的日期
            if prev_path and prev_path != filename:
                rx = re.compile(r"^stock_data_(?P<ts_code>.+)_(?P<date>\d{8})\.csv$")
                try:
                    prev_m = rx.match(os.path.basename(prev_path))
                    new_m = rx.match(os.path.basename(filename))
                    if prev_m and new_m and new_m.group("date") < prev_m.group("date"):
                        # 写入的是更早日期，保留原索引
                        pass
                    else:
                        _LATEST_CSV_INDEX[ts_code] = filename
                except Exception:
                    _LATEST_CSV_INDEX[ts_code] = filename
            else:
                _LATEST_CSV_INDEX[ts_code] = filename
...
def build_latest_csv_index(data_dir: str) -> Dict[str, str]:
    """
    扫描 data_dir 下的 stock_data_{ts_code}_{YYYYMMDD}.csv，构建 ts_code -> 最新文件路径 的索引。
    说明：
    - 仅匹配 8 位日期结尾的文件，避免误把 *_with_indicators.csv 之类纳入。
    - 同一 ts_code 多个日期时，取日期最大的；日期相同取 ctime 最新的。
    """
    import glob
    import os

    pattern = os.path.join(data_dir, "stock_data_*_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].csv")
    ...
    for path in files:
        ...
        if prev is None or (date_str, ctime) > (prev[0], prev[1]):
            best[ts_code] = (date_str, ctime, path)

    return {ts_code: item[2] for ts_code, item in best.items()}


def _get_latest_csv_index() -> Dict[str, str]:
    global _LATEST_CSV_INDEX
    # 双重检查避免并发下重复扫描目录
    if _LATEST_CSV_INDEX is not None:
        return _LATEST_CSV_INDEX
    with _LATEST_CSV_INDEX_LOCK:
        if _LATEST_CSV_INDEX is None:
            _LATEST_CSV_INDEX = build_latest_csv_index(file_dir_path)
        return _LATEST_CSV_INDEX


def load_stock_data_from_csv(ts_code, name, latest_csv_index: Optional[Dict[str, str]] = None):
    """
    从CSV文件加载股票数据
    """
    import os
    latest_csv_index = latest_csv_index or _get_latest_csv_index()
    latest_file = latest_csv_index.get(ts_code)
    if not latest_file:
        # 兼容兜底：如果文件命名不符合索引规则，退回到 glob 扫描
        import glob
...
def get_incremental_data(ts_code, name, start_date=None):
    ...
    recent_daily_vol_avg_hands: Optional[float] = None
    refresh_today_from_realtime = False
    ...
    if existing_df is not None and not existing_df.empty:
        ...
        # 关键逻辑：
        # - 如果本地已经有“今天”的数据，通常无需再拉
        # - 但如果今天这条数据来自 tushare_realtime_quotes，说明它只是盘中实时兜底快照，
        #   后续同一天再次执行时仍需要继续刷新当天数据
        if last_date == today_yyyy_mm_dd and "data_source" in existing_df.columns:
            today_rows = existing_df[existing_df["trade_date"].astype(str) == today_yyyy_mm_dd]
            if not today_rows.empty:
                data_sources = set(today_rows["data_source"].dropna().astype(str).tolist())
                refresh_today_from_realtime = "tushare_realtime_quotes" in data_sources
                if refresh_today_from_realtime:
                    logger.info(
                        "refresh_today_enabled ts_code=%s trade_date=%s reason=existing_today_from_realtime_quotes",
                        ts_code,
                        today_yyyy_mm_dd,
                    )
        ...
        if refresh_today_from_realtime:
            # 继续刷新“今天”，允许当天实时快照被新的实时/日线数据覆盖
            incremental_start_date = today_yyyy_mm_dd.replace("-", "")
        else:
    ...
    try:
        ...
        if not incremental_df.empty:
            ...
            if existing_df is not None and not existing_df.empty:
                ...
                # 关键修复：
                # 盘中同一天重复刷新时，existing_df 与 incremental_df 可能同时包含“今天”。
                # 必须先按 trade_date 去重（保留最新一条），再计算 KDJ/MACD/MA，
                # 否则同一天的重复行会进入滚动窗口，导致 KDJ 被算偏。
                combined_df = _normalize_trade_date_to_yyyy_mm_dd(combined_df)
...
def pick_stocks_union(
    *,
    ...
    # 预构建最新 CSV 索引：避免每个 worker 都 glob 扫描目录
    latest_csv_index = _get_latest_csv_index()

    def _worker_pick(stock_row: pd.Series) -> Tuple[str, Optional[Dict[str, Any]], Optional[str]]:
        ts_code = str(stock_row["ts_code"])
        name = str(stock_row["name"])
        try:
            df = load_stock_data_from_csv(ts_code, name, latest_csv_index=latest_csv_index)
            if df is None or df.empty:
...
if __name__ == "__main__":
    ...
    # python filter_stock_by_index.py pick-ma60-vol
        ...
        result = sort_results_by_ai_score(result)
